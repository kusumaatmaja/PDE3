...
def calculate_dt(dx: float, a: float, nu: float = 0.95) -> float:
    """Calculate the time step size $dt$ based on the grid spacing $dx$, the advection speed $a$, and the Courant number $nu$."""
    
    dt = nu * dx / np.abs(a)
    return dt

def solver(scheme, u_0: np.ndarray, a: float, dx: float, t_total: int):
